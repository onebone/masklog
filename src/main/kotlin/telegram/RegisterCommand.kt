			...
			if(it.isNotEmpty()) {
				if(this.bot.registerLocation(location, message.from.id, message.chat.id)) {
					replyMessage(sender, message,
						"주소(${location})가 등록되었습니다. 해당 도시에는 ${it.size}개의 공적 마스크 판매소가 있습니다." +
								" 마스크의 재고에 변화가 있을 경우 이곳에 메시지를 보냅니다.")
				}else{
					replyMessage(sender, message, "이미 해당 주소를 등록했습니다.")
				}
			}else{
				replyMessage(sender, message, "해당 주소는 올바르지 않습니다. 시 단위의 입력은 사용할 수 없습니다.\n올바른 사용 예시: 서울특별시 강남구")
			}
		})
